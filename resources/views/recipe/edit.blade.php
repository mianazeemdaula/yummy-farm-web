                    ...
                    <div class="row">
                        <table id="ingrdient-table" class="table table-sm">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <select class="form-control" name="ingredientcat[]">
                                            @foreach ($ingredientCategories as $cat)
                                            <option @if ($cat->id == $item->ingredient_category_id) selected @endif
                                                value="{{$cat->id}}">{{ $cat->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td><input class="form-control" type="text" name="ingredient[]"
                                            value="{{ $item->ingredient }}"></td>
                                    <td><input class="form-control" type="text" name="qty[]" value="{{ $item->qty }}">
                                    </td>
                                    <td>
                                        <select class="form-control" name="unit[]">
                                            @foreach ($units as $unit)
                                            <option @if ($unit->id == $item->ingredient_unit_id) selected @endif
                    ...
                    <div>
                        <ol id="insturctions" class="list-group">
                            ...
                            <li class="list-group-item">
                                <div class="row">
                                    <div class="col-md-11">
                                        <input value="{{ $instruction->instruction}}" type="text" class="form-control" name="instruction[]">
                                    </div>
                                    <a href="#" class="remove-instruction"><i class="fas fa-trash"></i></a>
                                </div>
                            </li>
                            @endforeach
                        </ol>
...
<script>
    $(document).ready(function(){
        ...
        $("#add-instruction").click(function(){
            ...
            $('#insturctions').append('<li class="list-group-item"><div class="row"><div class="col-md-11"><input value="'+instruction+'" type="text" class="form-control" name="instruction[]"></div><a href="#" class="remove-instruction"><i class="fas fa-trash"></i></a></div></li>');
